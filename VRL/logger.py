...
class Logger(object):
    ...
    def _try_sw_log_figure(self, key, value, step):
        if self._sw is not None:
            self._sw.add_figure(key, value, step)
    ...
    def log_figure(self, key, image, step):
        assert key.startswith('train') or key.startswith('eval')
        self._try_sw_log_figure(key, image, step)#plt.gcf())

    def log_metrics(self, metrics, step, ty):
        for key, value in metrics.items():
            if 'fig' in key:
                self.log_figure(f'{ty}/{key}', value, step)
            elif torch.is_tensor(value):
                if value.ndim > 2:
                    self.log_image(f'{ty}/{key}', value[0], step)
            else:
