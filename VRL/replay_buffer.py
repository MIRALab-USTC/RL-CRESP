...
class ReplayBuffer(Dataset):
    ...
    def __init__(self, obs_shape, state_shape, action_shape, capacity, buffer_dir, batch_size, device,
                  nstep=1, gamma=0.99, save_buffer=False, image_size=84, pre_image_size=84, **kwargs):
        obs_shape = (9, 84, 84)
    ...
    def sample_ablate(self, batch_size=None):
        ...
        if self.idx_epoch+batch_size > self.idx:
            obses = self.obses[self.idx_epoch:]
            obses = torch.as_tensor(obses, device=self.device).float()
            states = torch.as_tensor(self.states[self.idx_epoch:], device=self.device)
            env_labels = torch.as_tensor(self.env_labels[self.idx_epoch:], device=self.device)
            self.idx_epoch = 0
            done = True
        else:
        ...
        return obses, states, env_labels, self.idx_epoch, done

    def sample_val_ablate(self, batch_size=500):
        obses = self.obses_val[self.idx_val_epoch:self.idx_val_epoch+batch_size]
        obses = torch.as_tensor(obses, device=self.device).float()
        states = torch.as_tensor(self.states_val[self.idx_val_epoch:self.idx_val_epoch+batch_size], device=self.device)
        env_labels = torch.as_tensor(self.env_labels_val[self.idx_val_epoch:self.idx_val_epoch+batch_size], device=self.device)
        self.idx_val_epoch = (self.idx_val_epoch + batch_size) % self.idx_val
