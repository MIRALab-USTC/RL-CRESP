...
import utils
from drq import Actor, Critic, RandomShiftsAug, NoShiftAug
...
class CFPredictor(nn.Module):
    def __init__(self, obs_shape, feature_dim, action_shape, reward_dim, output_dim,
                 elite_output_dim, hidden_dim, activation=Swish, trunk=None, output_mode='min',
                 omega_num_sample=256, omega_output_mode=None, device='cpu'):
        super().__init__()
        self.feature_dim = feature_dim
        # Setting modules
        self.encoder = Encoder(obs_shape, feature_dim)
        input_dim = feature_dim + action_shape[0] * reward_dim + reward_dim
        self.predictor = nn.Sequential(nn.Linear(input_dim, hidden_dim),
    ...
    @property
    def omega(self):
        # output.shape == torch.Size([omega_num_sample, rew_seq_dim])
        log_std = torch.tanh(self.omega_logstd)
        log_std = LOG_SIG_MIN + log_std * (LOG_STD_MAX - LOG_SIG_MIN)
        std = torch.exp(log_std)
        noise = torch.randn(self.omega_num_sample, log_std.size(0)).to(log_std.device)
    ...
    def configure_optimizers(self, learning_rate, weight_decay):
        ...
        blacklist_weight_modules = (torch.nn.LayerNorm, torch.nn.Conv2d)
    ...
    def _concate(self, h, a_seq, w_seq):
        # a_seq:     torch.Size([batch_size, seq_dim, act_dim])
        #            --> torch.Size([batch_size, seq_dim * act_dim])
        if a_seq.ndim == 3:
            a_seq = a_seq.view(a_seq.size(0), -1)
        h_cat = torch.cat([h, a_seq], dim=-1)

        # w_seq:     torch.Size([num_sample, rew_seq_dim])
        #            --> torch.Size([num_sample, batch_size, rew_seq_dim])
        assert w_seq.ndim == 2, print(w_seq.ndim)
        batch_size = h.size(0)
        N, R = w_seq.size()
        w_seq = w_seq.unsqueeze(1).expand(N, batch_size, R)

        # if w_seq.ndim == 3:
        h_cat = h_cat.unsqueeze(0).expand(w_seq.size(0), *h_cat.size())
        return torch.cat([h_cat, w_seq], dim=-1)
    ...
    def forward(self, latent_state, action_sequence, omega_sequence):
        # latent_state:     torch.Size([batch_size, encoder_output_dim])
        # action_sequence:  torch.Size([batch_size, seq_dim, act_dim])
        # omega_sequence:   torch.Size([num_sample, rew_seq_dim])
        # inputs:           torch.Size([num_sample, batch_size, input_dim])
        # psi_cos/sin:      torch.Size([num_sample, batch_size, output_dim])
        # psi:              torch.Size([output_dim, num_sample, batch_size, 2])
        if latent_state.size(1) != self.feature_dim:
            latent_state = self.encoder.embed_state(latent_state)
        inputs = self._concate(latent_state, action_sequence, omega_sequence)
        psi_cos, psi_sin = self.predictor(inputs).chunk(2, -1)
    ...
    def save_snapshot(self, work_dir, best=False):
        ...
        payload = {'predictor': self.predictor, 'encoder': self.encoder}
...
class Agent:
    def __init__(self, obs_shape, action_shape, device, lr, feature_dim, hidden_dim,
                 ...
                 critic_target_update_freq=2, augmentation=RandomShiftsAug(pad=4), # CRESP
        ...
        # Initialize hyperparameters of CRESP
        ...
        self.cfpred = CFPredictor(obs_shape, feature_dim, action_shape, rsd_nstep, cf_output_dim,
                                  cf_elite_num, hidden_dim, Swish, None, cf_output_mode, 
                                  omega_num_sample, omega_cf_output_mode, device).to(device)

        self.encoder = self.cfpred.encoder
    ...
    def update_extr(self, obses, action_seqs, reward_seqs, step, cfp_opt=None):
        ...
        _, batch_size, _ = psi_targ.size()

        psi1 = self.cfpred(obs1, action_seqs, omega_seqs) # (cf_output_dim, num_sample, batch_size, 2)
        psi2 = self.cfpred(obs2, action_seqs, omega_seqs) # (cf_output_dim, num_sample, batch_size, 2)
        ...
        loss_psi_std = psi1_error.std(1).mean() + psi2_error.std(1).mean()

        # Cross Entropy
        psi_ce_targ = psi_targ.permute(1, 0, 2).reshape(batch_size, -1) # (batch_size, num_sample*2)
        psi1_ce = psi1.permute(0, 2, 1, 3).reshape(psi1.size(0), batch_size, -1) # (cf_output_dim, batch_size, num_sample*2)
        psi2_ce = psi2.permute(0, 2, 1, 3).reshape(psi2.size(0), batch_size, -1) # (cf_output_dim, batch_size, num_sample*2)
        ...
        if self.use_tb:
            ...
            if step % 100 == 0:
                ...
                plt.figure()
                temp = [plt.plot(np.arange(p.size(0)), p.detach().cpu().numpy()) for p in pe]
    ...
    def cresp(self, replay_buffer, step):
        metrics = dict()
    ...
    def update(self, replay_buffer, step):
        ...
        for i in range(self.update_times):
            ...
            if self.use_cf:
                metrics.update(self.cresp(replay_buffer, step+i))
